...
class RulesRouter
{
        ...
        foreach ($rules as $rule) {
            ...
            foreach ($rule['conditions'] as $condition) {
                ...
                switch ($type) {
                    ...
                    case Rules::RULE_TYPE_SOURCE: {
                            $allConditionsFulfilled &= $this->testSource($operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V1: {
                            $allConditionsFulfilled &= $this->testSourceVar('v1', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V2: {
                            $allConditionsFulfilled &= $this->testSourceVar('v2', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V3: {
                            $allConditionsFulfilled &= $this->testSourceVar('v3', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V4: {
                            $allConditionsFulfilled &= $this->testSourceVar('v4', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V5: {
                            $allConditionsFulfilled &= $this->testSourceVar('v5', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V6: {
                            $allConditionsFulfilled &= $this->testSourceVar('v6', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V7: {
                            $allConditionsFulfilled &= $this->testSourceVar('v7', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V8: {
                            $allConditionsFulfilled &= $this->testSourceVar('v8', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V9: {
                            $allConditionsFulfilled &= $this->testSourceVar('v9', $operator, $values);
                        }
                        break;

                    case Rules::RULE_TYPE_SOURCE_VAR_V10: {
                            $allConditionsFulfilled &= $this->testSourceVar('v10', $operator, $values);
                        }
                        break;
     ...
     * @return bool
     */
    protected function testSourceVar($var, $operator, $values)
    {
        require_once __DIR__ . '/../session/sessionClick.php';

        // try to get current source var form URL first, then from session
        $value = '';
        ...
        return $this->test($value, $operator, $values);
    }

    /**
     * @return bool
     */
    protected function testSource($operator, $values)
    {
        require_once __DIR__ . '/../session/sessionClick.php';
        require_once __DIR__ . '/../router.php';

        // try to get current source form router first, then from session
        $source = Router::getCurrentStatedSource();
        if ($source === null) {
            $session = new SessionClick();
            $clickInfo = $session->getClickInfo();
            if ($clickInfo) {
                $source = $clickInfo->getSource();
            } else {
                $source = '';
            }
        }

        return $this->test($source, $operator, $values);
    }
