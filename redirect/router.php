...
class Router
{
    ...
    static private $currentStatedSource = null;
    ...
    static function getCurrentStatedSource()
    {
        return self::$currentStatedSource;
    }
    ...
    function goToFinalURL($isOrganicView = false)
    {
        ...
        if ($slug) {
            if ($source) {
                self::$currentStatedSource = $source;
            } else {
                self::$currentStatedSource = null;
            }
                ...
                if (isset($linkSettings[Link::SETTINGS_HOOKS][Link::SETTINGS_HOOKS_REDIRECTS])) {
                    ...
                    if (isset($redirectHooks[Link::SETTINGS_HOOKS_REDIRECTS_PHP])) {
                        ...
                        foreach ($redirectHooks[Link::SETTINGS_HOOKS_REDIRECTS_PHP] as $phpHook) {
                            $code = $dynamicTokens->replace(base64_decode($phpHook), false);
                            RedirectHooks::executePHP($code);
                        }
                    }
                    ...
                    if (!$blockRedirect) {
                        if (isset($redirectHooks[Link::SETTINGS_HOOKS_REDIRECTS_HTML])) {
                            if ($linkSettings[Link::SETTINGS_REDIRECT_MODE] != Link::REDIRECT_MODE_CLOAKING && $linkSettings[Link::SETTINGS_REDIRECT_MODE] != Link::REDIRECT_MODE_VOLTIFY) {
                                $htmlContent = $link->getHTMLHooks(Link::HTML_HOOK_WHEN_BEFORE, true);
    ...
    static $allQueryVars = [
        self::QUERY_KEY_VAR_1,
        self::QUERY_KEY_VAR_2,
        self::QUERY_KEY_VAR_3,
        self::QUERY_KEY_VAR_4,
        self::QUERY_KEY_VAR_5,
        self::QUERY_KEY_VAR_6,
        self::QUERY_KEY_VAR_7,
        self::QUERY_KEY_VAR_8,
        self::QUERY_KEY_VAR_9,
        self::QUERY_KEY_VAR_10,
    ];
