...
class URLTools
{
    ...
    static function getCurrentScheme()
    {
        ...
        } else if (isset($_SERVER['HTTP_REFERER']) && isset($_SERVER['HTTP_HOST'])) {
            $url1 = $_SERVER['HTTP_REFERER'];
            $url2 = 'http://' . $_SERVER['HTTP_HOST'];
            if (self::isSameDomain($url1, $url2)) {
                return parse_url($url1, PHP_URL_SCHEME);
            }
        }
        return 'http';
    }
    ...
    static function getPluginURL($forceUpdate = false)
    {
        ...
        if (empty($url) || !is_string($url)) {
            if (function_exists('plugins_url')) {
                return plugins_url() . '/clickervolt';
            }
    ...
    static function fixMixedContent($url)
    {
        return self::getCurrentScheme() . '://' . self::removeScheme($url);
    }
    ...
    static function isSameDomain($url1, $url2)
    {
        $parts1 = parse_url($url1);
        $parts2 = parse_url($url2);
        return isset($parts1['host']) && isset($parts2['host']) && $parts1['host'] == $parts2['host'];
    }
