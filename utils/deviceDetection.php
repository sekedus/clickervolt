...
class DeviceDetection
{
    ...
    function __construct($engineType = self::ENGINE_PIWIK)
    {
        ...
        switch ($engineType) {
            ...
            default:
                throw new \Exception("Unknown engine '{$this->engineType}'");
        }
    }
    ...
    protected function resolvePiwik($userAgent)
    {
        ...
        if ($this->engine->isBot()) {
            ...
            $mappings = [
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_DESKTOP => self::DEVICE_TYPE_DESKTOP,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_SMARTPHONE => self::DEVICE_TYPE_SMARTPHONE,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_TABLET => self::DEVICE_TYPE_TABLET,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_FEATURE_PHONE => self::DEVICE_TYPE_FEATURE_PHONE,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_CONSOLE => self::DEVICE_TYPE_CONSOLE,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_TV => self::DEVICE_TYPE_TV,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_CAR_BROWSER => self::DEVICE_TYPE_CAR_BROWSER,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_SMART_DISPLAY => self::DEVICE_TYPE_SMART_DISPLAY,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_CAMERA => self::DEVICE_TYPE_CAMERA,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_PORTABLE_MEDIA_PAYER => self::DEVICE_TYPE_PORTABLE_MEDIA_PAYER,
                \DeviceDetector\Parser\Device\AbstractDeviceParser::DEVICE_TYPE_PHABLET => self::DEVICE_TYPE_PHABLET,
            ];
...
class PiwikCache implements \DeviceDetector\Cache\CacheInterface
{
    function fetch($id)
    {
        if (!$this->contains($id)) {
    ...
    function save($id, $data, $lifeTime = 0): bool
    {
        return file_put_contents($this->getFilePath($id), json_encode($data), LOCK_EX) !== false;
    }
