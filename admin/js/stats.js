class ClickerVoltStatsFunctions {
    ...
    static initStatsTable(settings) {
        ...
        var defaultSettings = {
            ...
            dataTableOptions: {
                ...
                footerCallback: function (row, data, start, end, display) {
                    ...
                    var indexClicks = ClickerVoltStatsFunctions.getColumnIndex($table, ClickerVoltStatsFunctions.COLUMN_CLICKS);
                    ...
                    var indexEPC = ClickerVoltStatsFunctions.getColumnIndex($table, ClickerVoltStatsFunctions.COLUMN_EPC);
                    var indexCPC = ClickerVoltStatsFunctions.getColumnIndex($table, ClickerVoltStatsFunctions.COLUMN_CPC);
                    ...
                    var EPC = totalClicks > 0 ? (totalRevenue / totalClicks).toFixed(4) : 0;
                    var CPC = totalClicks > 0 ? (totalCost / totalClicks).toFixed(4) : 0;
                    var ROI = ClickerVoltStatsFunctions.calculateROI(totalCost, totalProfit);

                    jQuery(api.column(indexEPC).footer()).text(EPC);
                    jQuery(api.column(indexCPC).footer()).text(CPC);
        ...
        var html = `<thead>
                        <tr>
                            ...
                            <th class='optional-metrics'>${ClickerVoltStatsFunctions.COLUMN_EPC}</th>
                            <th class='optional-metrics'>${ClickerVoltStatsFunctions.COLUMN_CPC}</th>
     ...
     * @return {int}
     */
    static getSegmentsCountFromRow(row) {
    ...
    static reformatStatsRows($table, rows, ajaxData) {
        ...
        if (rows.length > 0) {
            var nbSegments = ClickerVoltStatsFunctions.getSegmentsCountFromRow(rows[0]);
            ClickerVoltStatsFunctions.setOption($table, 'tree-max-level', nbSegments);

            if (nbSegments > 0) {
                ClickerVoltStatsFunctions.reformatGroupingActions(rows, ajaxData);
                ClickerVoltStatsFunctions.reformatSourceSegment(rows, ajaxData);
            }
     ...
     * @param {object} ajaxData 
     */
    static reformatSourceSegment(rows, ajaxData) {
        var segmentSourceKey = null;
        for (var i = 0; i < ajaxData.segments.length; i++) {
            if (ajaxData.segments[i] == clickerVoltVars.const.ReportingSegments.TYPE_SOURCE) {
                segmentSourceKey = `segment${i}`;
                break;
            }
        }
        if (segmentSourceKey) {
            for (var y = 0; y < rows.length; y++) {
                rows[y][segmentSourceKey] = '<span class="stats-segment-hint">source: </span>' + rows[y][segmentSourceKey];
            }
        }
    }
     ...
     * @param {object} ajaxData 
     */
    static reformatGroupingActions(rows, ajaxData) {
        if (ajaxData.options[clickerVoltVars.const.AjaxStats.OPTION_SEGMENT_ICONS]) {
        ...
        if (segmentIndex === 0) {
            ...
            var addCTRToChildren = function (rows) {
                for (var y = 0; y < rows.length; y++) {
                    var row = rows[y];
                    if (row['children']) {
                        var rootClicks = row['clicksUnique'];
                        if (rootClicks > 0) {
                            for (var i = 0; i < row['children'].length; i++) {
                                var childRow = row['children'][i];
                                ...
                                {
                                    var childClicks = childRow['clicksUnique'];
    ...
    static reformatStatsRow(row) {
        ...
        var nbSegments = ClickerVoltStatsFunctions.getSegmentsCountFromRow(row);
        ...
        // Push all segments
        var maxSegmentLength = 30;
        var suffixIfTooLong = '...';
        for (var i = 0; i < nbSegments; i++) {
            var value = '' + row[`segment${i}`];
            if (value.indexOf('<') == -1) {
                if (value.length > (maxSegmentLength + suffixIfTooLong.length)) {
                    var title = value;
                    var short = value.substr(0, maxSegmentLength);
                    value = `<span aria-label="${title}" data-balloon-pos="up-left">${short}${suffixIfTooLong}</span>`;
                }
            }
    ...
    static addSecondaryMetrics(row) {
        ...
        row['epc'] = clicks > 0 ? (revenue / clicks).toFixed(4) : 0;
        row['cpc'] = clicks > 0 ? (cost / clicks).toFixed(4) : 0;
...
ClickerVoltStatsFunctions.COLUMN_EPC = 'EPC';
ClickerVoltStatsFunctions.COLUMN_CPC = 'CPC';
