class AffiliateNetwork {
    ...
    constructor(name, tid, postback_subid, postback_transaction, postback_revenue) {
        ...
        this.ipn_integration_name = null;
    }

    asIPN(integration_name) {
        this.ipn_integration_name = integration_name;
        return this;
    }
    ...
    getPostbackURL() {
        var basePixel = clickerVoltVars.const.ConvPostbackURLTemplate;
        if (this.ipn_integration_name) {
            basePixel = clickerVoltVars.const.ConvIPNURLTemplate.replace('-INTEGRATION_NAME-', this.ipn_integration_name);
        }
        return ClickerVoltLinkController.replaceVarsFromConvPixel(basePixel, {
...
class AffiliateNetworkHelper {
    ...
    static getAllNetworks() {
        if (!AffiliateNetworkHelper.networks) {
            ...
            // Affiliate networks with S2S postback support
            ...
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("Awin", "clickref", "!!!clickRef!!!", "!!!transactionId!!!", "!!!commission!!!"));
            ...
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("CPALead", "subid", "{subid}", null, "{payout}"));
            ...
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("Creative Clicks", "s1", "#s1#", "", "#payout#"));
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("Digistore24", "cid", "{cid}", "{transaction_id}", "{amount_affiliate}"));
            ...
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("Market Health", "aff_sub", "{aff_sub}", "{transaction_id}", "{payout}"));
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("MaxBounty", "s1", "#S1#", "#OFFID#", "#RATE#"));
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("MaxWeb", "subid2", "{SUBID2}", "{ORDERID}", "{COMMISSION_AMOUNT}"));
            ...
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("Shareasale", "afftrack", "!!afftrack!!", "!!transID!!", "!!commission!!"));
            ...
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("YTZ", "s1", "{SUB1}", null, "{REVENUE}"));
            ...
            // AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("PayKickstart", "tid", "tracking_id", "product_name", "affiliate_commission_amount").asIPN("paykickstart"));
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("JVZoo", "tid", null, null, null).asIPN("jvzoo"));
            AffiliateNetworkHelper.registerNetwork(new AffiliateNetwork("WarriorPlus", "/[cid]", null, null, null).asIPN("warriorplus"));

            // Sort by affiliate network name
            var ordered = {};
            Object.keys(AffiliateNetworkHelper.networks).sort().forEach(function (key) {
                ordered[key] = AffiliateNetworkHelper.networks[key];
            });
            AffiliateNetworkHelper.networks = ordered;
        }
