...
require_once __DIR__ . '/../../db/tableCustomReports.php';
...
class AjaxStats extends Ajax
{
    ...
    static function getClickLog()
    {
        ...
        try {
            ...
            foreach ($sqlRows as $i => $sqlRow) {
                ...
                $actionType = $sqlRow['actionType'];
                $actionName = $sqlRow['actionName'];
                ...
                $row = [
                    ...
                    $isp, $ip, $source, $referrer, $actionRevenue, $actionType, $actionName, $duration->humanize($timeToAction),
     ...
     * 
     */
    static function getAllCustomReports()
    {
        $table = new TableCustomReports();
        $reports = $table->loadAll();
        foreach ($reports as $k => $report) {
            $reports[$k] = $report->toArray();
        }
        return $reports;
    }
     ...
     * 
     */
    static function saveCustomReports()
    {
        $id = empty($_POST['id']) ? null : $_POST['id'];
        $name = empty($_POST['name']) ? null : $_POST['name'];
        $linkId = empty($_POST['linkId']) ? null : $_POST['linkId'];
        $sourceId = empty($_POST['sourceId']) ? null : $_POST['sourceId'];
        $segments = empty($_POST['segments']) ? [] : $_POST['segments'];

        if (!$name) {
            throw new \Exception("Could not save custom report: name cannot be empty");
        }

        $report = new CustomReport($id, $name, $linkId, $sourceId, $segments);
        $table = new TableCustomReports();
     ...
     * 
     */
    static function deleteCustomReport()
    {
        $id = empty($_POST['id']) ? null : $_POST['id'];
        if (!$id) {
     ...
     * 
     */
    static function purgeAllStats()
    {
        self::purgeStats(TablePurgeableStats::CATEGORY_STATS_RAW);
        self::purgeStats(TablePurgeableStats::CATEGORY_STATS_SUMMARY);
    }

    /**
     * 
     */
    static function purgeRAWClicks()
    {
        self::purgeStats(TablePurgeableStats::CATEGORY_STATS_RAW);
    }

    /**
     * @param string $category - one of the TablePurgeableStats::CATEGORY_* constants
     */
    static private function purgeStats($category)
    {
        foreach (DB::$tableClasses as $class) {
            if (is_subclass_of($class, 'ClickerVolt\TablePurgeableStats')) {
                $obj = new $class;
                if ($obj->getPurgeableCategory() == $category) {
                    $obj->purge();
                }
            }
        }
