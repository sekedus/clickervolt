...
class AjaxLinks extends Ajax
{
    ...
    static function saveLink($form)
    {
        ...
        if ($fraudDetectionMode == Link::FRAUD_DETECTION_MODE_RECAPTCHA_V3 && !empty($fraudDetectionRecaptcha3SiteKey)) {
            ...
            $response['recaptcha'] = [
                'recaptcha3SiteKey' => $fraudDetectionRecaptcha3SiteKey,
                'recaptcha3SecretKey' => $fraudDetectionRecaptcha3SecretKey,
                'recaptcha3HideBadge' => $fraudDetectionRecaptcha3SecretKey,
            ];
        }
        ...
        $voltifyStaticContentReplacements = self::pairsToKeyValues($voltifyStaticContentReplacementsFrom, $voltifyStaticContentReplacementsTo, true);
     ...
     * @return array
     */
    static private function pairsToKeyValues(&$array1, &$array2, $allowEmptyValues = false)
    {
        $keyValues = [];

        foreach ($array1 as $i => $v1) {
            if ($v1 !== '') {
                if (array_key_exists($i, $array2) && ($allowEmptyValues || $array2[$i] !== '')) {
